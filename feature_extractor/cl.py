...
# build_graphs.pyより、i_classifierインスタンス作成時に呼び出される
class IClassifier(nn.Module):
    # バックボーンモデル(torchvision.modelsのresnet etc.)、特徴数、出力クラス数を指定
    # 対照学習後のモデルに指定したクラス数で出力FC層を繋げている。
    ...
    def forward(self, x):
        ...
        # feats.view(feats.shape[0],-1) = 特徴層からの出力を0次元目を保持して二次元に変換したテンソル
        # c = resnet出力からFCLでクラス数を変更した出力テンソル
